...
class AlarmControl:
    ...
    def __init__( self, config, mqtt_client):
        ...
        self.mqtt_client.message_callback_add( 'external/alarmclock/stopRinging',
             self.on_message_stopringing)
        self.mqtt_client.subscribe([
            ('external/alarmclock/stopRinging', 0),
    ...
    def start_ringing( self, alarm, now_time):
        ...
        if site.ringtone_status:
            ...
            self.mqtt_client.message_callback_add(
                'hermes/audioServer/{siteId}/playFinished'.format( siteId=site.siteid),
    ...
    def stop_ringing( self, site):
        ...
        self.mqtt_client.publish( 'external/alarmclock/ringingStopped',
            site.ringing_alarm.get_data_dict(), json=True)
    ...
    def on_message_sessionstarted( self, client, userdata, msg):
        ...
        if not self.config['snooze_config']['state'] and self.sites_dict[data['siteId']].session_pending:
            ...
            now_time = dt.now()
            text = _("Alarm is now ended.") + " " + _("It's {h}:{min} .").format(
